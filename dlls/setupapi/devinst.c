       ...
       DWORD flags, SP_DEVICE_INTERFACE_DATA *iface_data)
{
    ...
    WCHAR instance_id[MAX_PATH], *tmp;
    struct device_iface *iface;
    struct device *device;
    ...
    if (!SetupDiGetClassDevsExW(NULL, instance_id, NULL, DIGCF_DEVICEINTERFACE | DIGCF_PRESENT | DIGCF_ALLCLASSES,
            devinfo, NULL, NULL))
        return FALSE;
...
BOOL WINAPI SetupDiOpenDeviceInterfaceA(HDEVINFO devinfo, const char *device_path,
       DWORD flags, SP_DEVICE_INTERFACE_DATA *iface_data)
{
    WCHAR device_pathW[MAX_PATH];

    TRACE("%p %s %#lx %p\n", devinfo, debugstr_a(device_path), flags, iface_data);

    if (!device_path || strlen(device_path) >= MAX_PATH)
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }

    MultiByteToWideChar(CP_ACP, 0, device_path, -1, device_pathW, ARRAY_SIZE(device_pathW));
    return SetupDiOpenDeviceInterfaceW(devinfo, device_pathW, flags, iface_data);
}
...
CONFIGRET WINAPI CM_Locate_DevNode_ExA(DEVINST *devinst, DEVINSTID_A device_id, ULONG flags, HMACHINE machine)
{
    ...
    if (!(device_idw = malloc((strlen(device_id) + 1) * sizeof(*device_idw))))
