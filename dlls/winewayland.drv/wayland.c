...
BOOL wayland_process_init(void)
{
    ...
    /* The color-management handshake needs several extra round-trips beyond
     * the two above: get_image_description -> ready -> get_information ->
     * info events -> done. Drain them now so the synthetic EDID generated by
     * the initial display device enumeration carries the HDR static metadata
     * (notably the desired content max luminance). Otherwise an early HDR
     * capability query (e.g. a D3D12 game probing IDXGIOutput6 at startup)
     * sees a zero max luminance and falls back to the generic default coded
     * in DXVK's DXGI implementation, which is 1499.0 nits for HDR outputs.
     *
     * In practice, all compositors examined, which is KWin, wlroots, and
     * Mutter, require the same number of round-trips to feasibly set this
     * up, but both KWin and wlroots, per their current designs, eagerly
     * fire off the preferred format signal on surface mapping, since they
     * both assume a default format of sRGB and advertise otherwise
     * immediately. Mutter deems immediate advertisement to be a potential
     * race condition with clients which may request it proactively, or this
     * may be a different issue altogether. Either way, in practice, Mutter
     * requires one more round-trip before this ready signal would fire and
     * clear this loop in one pass. I think it's a hack to simply add a hard
     * coded round-trip when a readiness check is sufficient to fix it, and
     * loops are added to prevent an unlikely worst case scenario from going
     * out of control and hanging the startup. */
    if (process_wayland.wp_color_manager_v1)
    {
        int i;
        for (i = 0; i < 8 && !wayland_outputs_color_management_done(); i++)
            wl_display_roundtrip_queue(process_wayland.wl_display,
                                       process_wayland.wl_event_queue);
        TRACE("Wayland color-management took %d additional round-trips to sync%s", i, (i == 8) ? " (the loop overflowed without achieving readiness!)" : "");
    }
